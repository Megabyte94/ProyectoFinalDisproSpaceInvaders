...
void detectPlayerBulletHits(Alien aliens[], int totalAliens, int width, int height, int frameToggle);
...
int kbhit(void);
void detectAlienBulletHitsPlayer(Player *player);
...
int main() {
    ...
    while( 1 ) {
        ...
        if (fps >= 60000) {
            ...
            updatePlayerBullets();
            detectPlayerBulletHits(aliens, TOTAL_ALIENS, width, height, frameToggle); 
            detectBulletCollisions();
            detectAlienBulletHitsPlayer(&player);
...
int checkCollision(int ax, int ay, int aw, int ah, int bx, int by, int bw, int bh) {
    return !(ax + aw - 1 < bx ||   // Alien completamente a la izquierda
             ax > bx + bw - 1 ||   // Alien completamente a la derecha
             ay + ah - 1 < by ||   // Alien completamente arriba
             ay > by + bh - 1);    // Alien completamente abajo
}
...
void detectPlayerBulletHits(Alien aliens[], int totalAliens, int width, int height, int frameToggle) {
    for(int b = 0; b < MAX_PLAYER_BULLETS; b++) {
        ...
        for(int a = 0; a < totalAliens; a++) {
            if(aliens[a].x < 0) continue; // alien destruido
            ...
            if(checkCollision(
                ...
                printf("\x1b[31m");      // activar rojo
                drawElement(kaboom, aliens[a].x, aliens[a].y);
                printf("\x1b[0m");       // reset de color

                
                Delay_Seconds(0.5); // tiempo que se muestra la explosión
                clearElement( kaboom, aliens[a].x, aliens[a].y );

                // Marcar alien destruido
                aliens[a].x = -100;  // fuera de pantalla
                aliens[a].y = -100;
                break;
                }
            }
...
void detectBulletCollisions() {
    for(int i = 0; i < MAX_PLAYER_BULLETS; i++) {
        ...
        for(int j = 0; j < MAX_ALIEN_BULLETS; j++) {
            ...
            // Usamos checkCollision: playerBullet = A, alienBullet = B
            if(checkCollision(
                playerBullets[i].x, playerBullets[i].y, 5, 5,   // ancho y alto de bala jugador
                alienBullets[j].x, alienBullets[j].y, 5, 5     // ancho y alto de bala alien
            )) {
                // -------------------- Borrar ambas balas --------------------
                for(int l = 0; l < 5; l++) {
                    gotoxy(playerBullets[i].x, playerBullets[i].y + l);
...
void detectAlienBulletHitsPlayer(Player *player) {
    for(int i = 0; i < MAX_ALIEN_BULLETS; i++) {
        ...
        // Verificar colisión usando checkCollision
        if(checkCollision(
            alienBullets[i].x, alienBullets[i].y, 5, 5,      // ancho y alto bala alien
            player->x, player->y, PLAYER_WIDTH, PLAYER_HEIGHT // ancho y alto jugador
        )) {
            // -------------------- Borrar bala --------------------
            for(int l = 0; l < 5; l++) {
                gotoxy(alienBullets[i].x, alienBullets[i].y + l);
            ...
            // -------------------- Aquí puedes agregar efecto jugador "hit" o perder vida --------------------
            // Por ejemplo: printExplosion(player->x, player->y);

            break; // solo una bala puede impactar por iteración
        }
    }
}
