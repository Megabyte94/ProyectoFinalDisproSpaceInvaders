...
PlayerBullet playerBullet = {0, 0, 0};
...
int main() {
    ...
    while (1) {   // se repite para siempre
        ...
        if (kbhit()) {
            ...
            if (tecla == 'a' && playerX > 1) {
                clearElement(playerSprite, playerX, playerY);
                playerX--;
            }

            // mover a la derecha
            if (tecla == 'd' && playerX < SCREEN_WIDTH - 6) {
                clearElement(playerSprite, playerX, playerY);
                playerX++;
            }

            // DISPARAR
            if (tecla == ' ' && bulletActive == 0) {
                bulletActive = 1;
                bulletX = playerX + 2;   // centro del cañón
                bulletY = playerY - 1;
            }
        }

        // -------- DIBUJAR JUGADOR --------
        drawElement(playerSprite, playerX, playerY);


        // -------- DISPARO --------
        if (bulletActive) {
            clearElement(bulletSprite, bulletX, bulletY);
            bulletY--;
            ...
            if (bulletY < 1) {
                ...
                drawElement(bulletSprite, bulletX, bulletY);
            }
        }


        // -------- MOVIMIENTO DEL ALIEN --------
        clearElement(alienSprite, alienX, alienY);
        alienY++;

        // si llega al fondo, vuelve a empezar arriba
        if (alienY > SCREEN_HEIGHT - 2) {
            alienY = 1;
            alienX = rand() % (SCREEN_WIDTH - 6);
        }

        drawElement(alienSprite, alienX, alienY);


        // -------- COLISIÓN BALA – ALIEN --------
        if (bulletActive &&
            bulletY == alienY &&
            (bulletX >= alienX && bulletX <= alienX + 4)) {

            clearElement(alienSprite, alienX, alienY);

            // nuevo alien en parte superior
            alienY = 1;
            alienX = rand() % (SCREEN_WIDTH - 6);

            bulletActive = 0; // bala desaparece
        ...
        usleep(60000); // velocidad de actualización
    }
...
void updatePlayerBullet()
{
    ...
    // borrar bala en la posición actual
    gotoxy(playerBullet.x, playerBullet.y);
    printf(" ");

    // subir bala
    playerBullet.y--;

    // si llegó arriba, se desactiva
    if (playerBullet.y < 2) {
        playerBullet.active = 0;
        return;
    }

    // volver a dibujar bala
    gotoxy(playerBullet.x, playerBullet.y);
    printf("|");
}
...
void handlePlayerMovement(Player *p, int screenWidth)
{
    /* Si no hay tecla, kbhit() devuelve 0 y no hacemos nada */
    if (!kbhit()) return;

    int ch = getchar();

    /* Para flechas (getch devuelve 0 o 224 antes) */
    if (ch == 0 || ch == 224) {
        int ch2 = getchar();
        ...
        if (ch == 'a' || ch == 'A') {
            ...
            /* Punto de integración para disparo del jugador:
               createPlayerBullet(p->x + PLAYER_WIDTH/2, p->y - 1);
               Por ahora solo dejamos la detección de la tecla. */
        }
    }
}
...
{
    // Si ya hay una bala activa, no disparamos otra
    if (playerBullet.active) return;

    // Activar bala y posicionarla en la punta del cañón
    playerBullet.active = 1;
    playerBullet.x = p->x + 2;  // centro del sprite del jugador
    playerBullet.y = p->y - 1;  // justo arriba
}
